...
--조회시 index를 통해 조회하려면 반드시 where조건에 해당 칼럼이 있어야 함
select * from author where email = 'honglidong';


--id에 index가 걸려있으므로 id의 index를 통해 조회 후 name full scan
selecet * from author where id =1 and name = 'hongildong'

--만약에 두 컬럼에 동시에 index가 걸려있다면 해당 index활용
--만약에 두 컬럼 각각 index가 설정되어 있다면 mariaDB 엔진에서 최적의 알고리즘을 실행해 알아서 찾는다
